export const SustainabilityEnvironmentProduction = {
  bannerTitle: "綠色創新",
  bannerContent:`從綠色產品之設計、供應鏈到生產，秉持企業成長與生態環境共榮的信念，厚植綠色管理於日常營運。`,
  title1:"永續生產",
  title2:"2021年瀚宇彩晶環境足跡",
  title3:"永續案例",
  subTitle2:"彩晶推動高效能、低汙染的綠色生產，達到企業獲利與環境永續的雙贏目標。面對全球暖化及極端氣候帶來可能的營運衝擊進行廠區的節能減碳，並揭露2021年瀚宇彩晶環境足跡。",
  cardTitle1:`100%使用FCs機導入NF3及SF6燃燒減量設備`,
  cardContent1:`溫室氣體排放自2005年累積至2021年已減少1,928,165公噸CO2e。`,
  cardTitle2:`Array光阻液回收`,
  cardContent2:`將製程中塗佈後多餘的光阻液收集，2021年回收再利用65.07公噸光阻液。`,
  cardTitle3:`再生水工程改善`,
  cardContent3:`降低製造再生水後清洗管路的耗水量，預估每年可節省36,500百萬公升用水。`,
  breadcrumbsItem1:"企業永續",
  breadcrumbsItem2:"永續生產"
}